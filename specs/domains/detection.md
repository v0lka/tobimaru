...
- `NewEngine(cfg)` — creates engine with buffered alerts channel and dedup map. Defensively falls back to `DefaultAlertBufferSize` and `DefaultDedupWindow` when config values are zero/non-positive.
- `Register(rule)` — validates rule name uniqueness and calls `rule.Init(cfg)`; returns `ErrEngineStarted` if `Run()` has already been called, or error on duplicate name or init failure
- `RuleCount()` — returns the number of registered detection rules
- `Run(ctx, frames)` — starts dispatch goroutine (reading frames, dispatching to rules, deduplicating, emitting alerts). Returns immediately. Idempotent: subsequent calls are no-ops (`sync.CompareAndSwap`). Goroutine exits on ctx cancellation or frames channel close.
- `Alerts()` — returns read-only alerts channel; closes when dispatch goroutine exits
- `SetEnabled(v bool)` — atomically toggles alert emission; engine still processes frames for rule state even when disabled
- `Enabled() bool` — reports current enabled state
- `SetDedupWindow(d time.Duration)` — atomically updates deduplication window (≤0 values ignored)
- `DedupWindow() time.Duration` — returns the current deduplication window

`ErrEngineStarted` is a sentinel error returned by `Register` when the engine has already been started. It is exported so that callers (e.g., tests) can distinguish between configuration errors and ordering errors.
...
- `Register()` must be called before `Run()`; the rule list is immutable after `Run()` is called. `Run()` is idempotent — subsequent calls log a warning and return immediately.
- Duplicate rule names are rejected at registration time
- Rules that fail `Init()` are not added to the engine
- Runtime enable/disable via `SetEnabled()` suppresses event emission but keeps processing frames for rule state (e.g., beacon baseline learning)
- Dedup window can be changed at runtime via `SetDedupWindow()` without restarting the engine
