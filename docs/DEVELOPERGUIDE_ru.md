...
1. Разбор флагов CLI (`-config`, `-version`, `-hash-password`).
   Флаг `-hash-password` — это отдельная утилита: она запрашивает пароль
   интерактивно, генерирует bcrypt-хэш, выводит его в stdout и немедленно
   завершается — демон не запускается. Пароль считывается через терминал
   (без отображения на экране), поэтому он не появляется в списке процессов
   или истории командной оболочки. Так оператор получает значения
   `admin_password_hash`/`user_password_hash` для YAML-конфига.
